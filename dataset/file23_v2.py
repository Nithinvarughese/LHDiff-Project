...
def fetch(page_no, verbose=False):
    ...
    if verbose:
        print(f'Fetching Page {i}...')   # small change
    ...
    try:
        ...
        file_path = os.path.join('HackerNews', f'NewsPage{i}.txt')   # split part 1
        with open(file_path, 'w+') as f:                              # split part 2
            f.write('-' * 80)
            f.write('\n')
            ...
            for idx in range(num_iter):
                ...
                url = (
                    titl['href'] if titl and titl['href'].startswith('https')
                ...
                posted_time = tdm[idx].find('span', attrs={'class': 'age'})  # renamed
                author = tdm[idx].find('a', attrs={'class': 'hnuser'})
                f.write(
                    '\nArticle Number: ' +
                    rank.text.replace('.', '') if rank
                ...
                f.write(
                    '\nPosted: ' +
                    posted_time.text if posted_time
                    else '\nPosted: Could not find when the article was posted'
                )

                f.write('\n' + '-' * 80 + '\n')

    except (requests.ConnectionError, requests.packages.urllib3.exceptions.ConnectionError):
