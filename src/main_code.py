    ...
    def analyze_bug_fixes(self, commits: List[Commit]) -> List[BugIntroducingChange]:
        ...
        for fix_commit in bug_fixes:
            ...
            for file_path in fix_commit.files_changed:
                # Skip analyzing this script itself
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Detect bug-introducing changes from git history")
    parser.add_argument('--repo', default='.', help='Path to git repository (default: current directory)')
    parser.add_argument('--max-commits', type=int, default=100, help='Maximum number of commits to analyze')
    parser.add_argument('--output', default='bonus', help='Output directory')
